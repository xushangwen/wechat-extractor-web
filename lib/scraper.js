...
export async function scrapeArticle(url) {
  ...
  if (isVercel) {
    ...
    browser = await puppeteer.launch({
      args: [
        ...chromium.args,
        '--single-process',  // 绕过 zygote 进程（避免 libnss3.so 缺失问题）
        '--no-zygote',
      ],
    ...
    function extractImageContext(imgNode) {
      ...
      while (ancestor && ancestor !== content && depth < 8) {
        ...
        if (nextSib && !nextSib.querySelector('img')) {
          ...
          // 注释特征：非空、短（≤80字符）、不含嵌套 section 列表
          if (text && text.length > 0 && text.length <= 80
              && nextSib.querySelectorAll('section').length < 3) {
            context.caption = text;
            break;
          }
        }
      ...
      // 3. 前后文本：找到最顶层祖先（js_content 的直接子节点），取其兄弟节点文本
      let topAncestor = imgNode.parentElement;
      while (topAncestor && topAncestor.parentElement
             && topAncestor.parentElement !== content) {
      ...
      if (topAncestor) {
        ...
        if (prevSib && !prevSib.querySelector('img')) {
          context.before = prevSib.textContent.trim().substring(0, 150);
        }
        if (nextSibCtx && !nextSibCtx.querySelector('img')) {
          context.after = nextSibCtx.textContent.trim().substring(0, 150);
        }
      }

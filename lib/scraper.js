...
async function extractArticleStructure(page) {
  return await page.evaluate(() => {
    ...
    function extractImageContext(imgNode) {
      ...
      // 1. 检查图片的 alt 属性作为 caption
      if (imgNode.alt && imgNode.alt.trim()) {
        context.caption = imgNode.alt.trim();
      }
      
      // 2. 检查父元素是否有 figcaption 或类似的说明文字
      let parent = imgNode.parentElement;
      while (parent && parent !== content) {
        const caption = parent.querySelector('figcaption, .img-desc, .img-caption, .image-caption');
      ...
      // 3. 提取图片前后的文本节点
      const collectText = (node, direction, maxChars = 200) => {
        let text = '';
        let current = node;
        
        while (current && text.length < maxChars) {
          current = direction === 'before' ? current.previousSibling : current.nextSibling;
          if (!current) {
            current = direction === 'before' ? node.parentElement?.previousSibling : node.parentElement?.nextSibling;
          }
          if (!current) break;
          
          if (current.nodeType === 3) { // Text node
            const t = current.textContent.trim();
            if (t) text = direction === 'before' ? t + ' ' + text : text + ' ' + t;
          } else if (current.nodeType === 1) { // Element node
            const t = current.textContent.trim();
            if (t && !current.querySelector('img')) {
              text = direction === 'before' ? t + ' ' + text : text + ' ' + t;
              break; // 找到一个完整段落就停止
            }
          }
        }
      ...
      context.before = collectText(imgNode, 'before');
      context.after = collectText(imgNode, 'after');

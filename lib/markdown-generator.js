    ...
    switch (el.type) {
      ...
      case 'image': {
        ...
        if (imgInfo) {
          ...
          // 图片描述使用小字体
          if (imgInfo.description && imgInfo.description !== '图片') {
            lines.push(`<sub>${imgInfo.description}</sub>`);
          }
          lines.push('');
        }
        break;
        ...
        if (el.images) {
          for (const gImg of el.images) {
            ...
            if (imgInfo) {
              ...
              lines.push(`![${desc}](images/${imgInfo.finalFilename})`);
              if (imgInfo.description && imgInfo.description !== '轮播图') {
                lines.push(`<sub>${imgInfo.description}</sub>`);
              }
            }
          }
      ...
      case 'background_image': {
        ...
        if (imgInfo && !imgInfo.isDecorative) {
          ...
          if (imgInfo.description && imgInfo.description !== '背景图') {
            lines.push(`<sub>${imgInfo.description}</sub>`);
          }
          lines.push('');
        }

...
function buildAnalysisPrompt(articleTitle, imagesWithContext) {
  ...
  // 为每张图片添加上下文信息
  // 注意：context 存储在 img.element.context（来自 downloadAllImages 的 task 结构）
  imagesWithContext.forEach((img, idx) => {
    const ctx = img.element?.context || img.context;
    if (ctx) {
      let ctxText = `\n图片 ${idx}`;
...
- 有疑问时，倾向于标记为 content（宁可多保留，不要误删）

**特别注意**：
- 活动/会议主题海报、年会主题横幅、产品发布图 → content（即使是纯文字排版）
- 带有图片说明文字（context 中提供了图片说明）的图片 → 优先标记为 content
